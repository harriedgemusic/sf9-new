...
import urllib.parse
import subprocess
from providers import Downloader, SongLink
import difflib
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"ok": False, "error": "Usage: simple_dl.py <fetch|download|download_url> ..."}))
        return 1
    ...
    if cmd == "fetch":
        if len(sys.argv) < 3:
            print(json.dumps({"ok": False, "error": "Usage: simple_dl.py fetch <url>"}))
            return 1
        return cmd_fetch(sys.argv[2])

    if cmd == "download":
        if len(sys.argv) < 4:
            print(json.dumps({"ok": False, "error": "Usage: simple_dl.py download <track_json> <output_dir>"}))
            return 1
